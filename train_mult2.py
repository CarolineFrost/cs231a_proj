...
if __name__ == '__main__':
    ...
    L_style1 = L_style(session, model, style_weights)
    style = init_image('style/temp1.jpg')
    session.run(model['init'].assign(style))
    L_style2 = L_style(session, model, style_weights)
    style = init_image('style/temp2.jpg')
    session.run(model['init'].assign(style))
    L_style3 = L_style(session, model, style_weights)
    ...
    total_loss = alpha * L_content + beta * (L_style1 + L_style2 + L_style3) / 3
    ...
    for it in range(ITERATIONS+1):
        ...
        if it%10 == 0:
            ...
            if not os.path.exists('output'):
                os.mkdir('output')
            filename = 'output/mult2_%d.png' % (it)
